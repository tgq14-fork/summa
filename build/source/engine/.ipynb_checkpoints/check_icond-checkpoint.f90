...
contains
 ...
 do iGRU = 1,nGRU
  do iHRU = 1,gru_struc(iGRU)%hruCount
   ...
   ! WK-print
   !print*,'In check_icond()'
   !print*,'nLayers = ',nLayers
   !print*,'nSnow   = ',nSnow
   !print*,'mLayerLayerType = ', mlayerLayerType
   ...
   do iLayer=1,nLayers
	...
	select case(mlayerLayerType(iLayer))
     ...
     case(iname_snow)
      ...
      ! (check ice)
      if(mLayerVolFracIce(iLayer) > 0.80_rkind)then; write(message,'(a,1x,i0,i0,f10.2)') trim(message)//'cannot initialize the model with volumetric fraction of ice > 0.80: layer, layerType, ice fraction = ',iLayer,mlayerLayerType(iLayer),mLayerVolFracIce(iLayer); err=20; return; end if
      if(mLayerVolFracIce(iLayer) < 0.05_rkind)then; write(message,'(a,1x,i0,i0,f10.2)') trim(message)//'cannot initialize the model with volumetric fraction of ice < 0.05: layer, layerType, ice fraction = ',iLayer,mlayerLayerType(iLayer),mLayerVolFracIce(iLayer); err=20; return; end if
      ! check total water
