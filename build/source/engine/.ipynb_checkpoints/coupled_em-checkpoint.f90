...
contains
 ...
 dt_init = min(data_step,maxstep) / 4 ! initial substep length (s)
 ...
 substeps: do  ! continuous do statement with exit clause (alternative to "while")
  ...
  ! WK-print
  !print*,' -- after opSplittin --'
  !print*,'stepFailure          = ', stepFailure
  !print*,'scalarInfiltration   (data) = ', flux_data%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step
  !print*,'iLayerLiqFluxSoillux (data) = ', flux_data%var(iLookFLUX%iLayerLiqFluxSoil)%dat    *iden_water*data_step
  ...
  !print*,'scalarInfiltration        (data) = ', flux_data%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step
  !print*,'iLayerLiqFluxSoil         (data) = ', flux_data%var(iLookFLUX%iLayerLiqFluxSoil)%dat    *iden_water*data_step
  !print*,'scalarSoilBaseflow        (data) = ', flux_data%var(iLookFLUX%scalarSoilBaseflow)%dat(1)*iden_water*data_step
  !print*,'scalarSoilDrainage        (data) = ', flux_data%var(iLookFLUX%scalarSoilDrainage)%dat(1)*iden_water*data_step
  !print*,'scalarCanopyTranspiration (data) = ', flux_data%var(iLookFLUX%scalarCanopyTranspiration)%dat(1)*iden_water*data_step
  !print*,'scalarGroundEvaporation   (data) = ', flux_data%var(iLookFLUX%scalarGroundEvaporation)  %dat(1)*iden_water*data_step
  !print*,'soil water balance error         = ', (sum(prog_data%var(iLookPROG%mLayerVolFracLiq)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water) + sum(prog_data%var(iLookPROG%mLayerVolFracIce)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water)) - (balanceSoilWater0 + (flux_data%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step + flux_data%var(iLookFLUX%scalarCanopyTranspiration)%dat(1)*iden_water*data_step + flux_data%var(iLookFLUX%scalarGroundEvaporation)%dat(1)*iden_water*data_step - flux_data%var(iLookFLUX%scalarSoilBaseflow)%dat(1)*iden_water*data_step - flux_data%var(iLookFLUX%scalarSoilDrainage)%dat(1)*iden_water*data_step - diag_data%var(iLookDIAG%scalarSoilCompress)%dat(1)) )
  ...
  do iVar=1,size(averageFlux_meta)
   !print*,'copying ',averageFlux_meta(iVar) ! WK-print -> this section only finds averages for 'scalarX' variables. NOT for 'iLayerX' or 'mLayerX' variables
   flux_mean%var(iVar)%dat(:) = flux_mean%var(iVar)%dat(:) + flux_data%var(averageFlux_meta(iVar)%ixParent)%dat(:)*dt_wght
  end do
  
  ! WK-print
  !print*,' -- after flux weighting --'
  !print*,'substep weight dt_wght = ', dt_wght 
  !print*,'mLayerVolFracLiq                 = ', prog_data%var(iLookPROG%mLayerVolFracLiq)%dat(nSnow+1:nSoil+nSnow)
  !print*,'mLayerVolFracIce                 = ', prog_data%var(iLookPROG%mLayerVolFracIce)%dat(nSnow+1:nSoil+nSnow)
  !print*,'soilBalance0                     = ', balanceSoilWater0
  !print*,'new scalarTotalSoilLiq           = ', sum(prog_data%var(iLookPROG%mLayerVolFracLiq)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water)
  !print*,'new scalarTotalSoilIce           = ', sum(prog_data%var(iLookPROG%mLayerVolFracIce)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water)
  !print*,'soilBalance1                     = ', sum(prog_data%var(iLookPROG%mLayerVolFracLiq)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water) + sum(prog_data%var(iLookPROG%mLayerVolFracIce)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water)
  !print*,'scalarSoilCompress               = ', diag_data%var(iLookDIAG%scalarSoilCompress)%dat(1)
  !print*,'scalarInfiltration        (mean) = ', flux_mean%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step
  !print*,'iLayerLiqFluxSoil         (mean) = ', flux_mean%var(iLookFLUX%iLayerLiqFluxSoil)%dat    *iden_water*data_step
  !print*,'scalarSoilBaseflow        (mean) = ', flux_mean%var(iLookFLUX%scalarSoilBaseflow)%dat(1)*iden_water*data_step
  !print*,'scalarSoilDrainage        (mean) = ', flux_mean%var(iLookFLUX%scalarSoilDrainage)%dat(1)*iden_water*data_step
  !print*,'scalarCanopyTranspiration (mean) = ', flux_mean%var(iLookFLUX%scalarCanopyTranspiration)%dat(1)*iden_water*data_step
  !print*,'scalarGroundEvaporation   (mean) = ', flux_mean%var(iLookFLUX%scalarGroundEvaporation)%dat(1)*iden_water*data_step
  !print*,'soil water balance error         = ', (sum(prog_data%var(iLookPROG%mLayerVolFracLiq)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water) + sum(prog_data%var(iLookPROG%mLayerVolFracIce)%dat(nSnow+1:nSoil+nSnow)*prog_data%var(iLookPROG%mLayerDepth)%dat(nSnow+1:nSoil+nSnow)*iden_water)) - (balanceSoilWater0 + (flux_mean%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step + flux_mean%var(iLookFLUX%scalarCanopyTranspiration)%dat(1)*iden_water*data_step + flux_mean%var(iLookFLUX%scalarGroundEvaporation)%dat(1)*iden_water*data_step - flux_mean%var(iLookFLUX%scalarSoilBaseflow)%dat(1)*iden_water*data_step - flux_mean%var(iLookFLUX%scalarSoilDrainage)%dat(1)*iden_water*data_step - diag_data%var(iLookDIAG%scalarSoilCompress)%dat(1)) )
 ...
 ! WK-print
 !print*,' -- after data structure update --'
 !print*,'scalarInfiltration   (mean) = ', flux_mean%var(childFLUX_MEAN(iLookFLUX%scalarInfiltration))%dat(1)*iden_water*data_step
 !print*,'scalarInfiltration   (data) = ', flux_data%var(iLookFLUX%scalarInfiltration)%dat(1)*iden_water*data_step
 !print*,'iLayerLiqFluxSoillux (mean) = ', flux_mean%var(childFLUX_MEAN(iLookFLUX%iLayerLiqFluxSoil))%dat    *iden_water*data_step
 !print*,'iLayerLiqFluxSoillux (data) = ', flux_data%var(iLookFLUX%iLayerLiqFluxSoil)%dat    *iden_water*data_step
 ...
 ! check the individual layers
 if(printBalance)then
  print*,'-- individual soil layers --'
  write(*,*)                     'nSnow             = ', nSnow
  write(*,*)                     'nSoil             = ', nSoil
  write(*,'(a,1x,10(f12.8,1x))') 'liqSoilInit       = ', liqSoilInit
  write(*,'(a,1x,10(f12.8,1x))') 'mLayerVolFracLiq  = ', prog_data%var(iLookPROG%mLayerVolFracLiq)%dat
  write(*,'(a,1x,10(f12.8,1x))') 'mLayerVolFracIce  = ', prog_data%var(iLookPROG%mLayerVolFracIce)%dat
  write(*,'(a,1x,10(f12.8,1x))') 'mLayerLiq         = ', mLayerVolFracLiq*iden_water*mLayerDepth
  write(*,'(a,1x,10(f12.8,1x))') 'mLayerIce         = ', mLayerVolFracIce*iden_ice*mLayerDepth
 ...
 if(abs(scalarSoilWatBalError) > absConvTol_liquid*iden_water*10._rkind)then  ! NOTE: kg m-2, so need coarse tolerance to account for precision issues
  ...
  write(*,'(a,1x,f20.10)') 'averageGroundEvaporation  = ', averageGroundEvaporation * data_step
  write(*,'(a,1x,f20.10)') 'averageCanopyTranspiration= ', averageCanopyTranspiration * data_step
